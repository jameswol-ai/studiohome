...
def render():
    ...
    # Radar chart for civilization state using Plotly
    radar_df = pd.DataFrame({
        "Metric": ["Civic Stability", "Innovation Drive", "Cultural Synergy", "Resource Resilience", "Diplomatic Trust"],
        "Score": [cs['stability'], cs['innovation'], cs['culture_score'], 0.85, 0.79]
    })
    
    fig = px.line_polar(radar_df, r="Score", theta="Metric", line_close=True, range_r=[0, 1], title="Collective Civilization Equilibrium Radar", template="plotly_dark", height=340)
