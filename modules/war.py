...
def render():
    st.markdown("## 🛡️ Strategic Conflict & Defense Simulation")
    ...
    if st.button("⚔️ Simulate Strategic Engagement", use_container_width=True):
        ...
        st.json({
            "aggressor_force_attrition": f"{random.randint(6, 24)}%",
            "defender_infrastructure_impact": f"{random.randint(3, 16)}%",
            "treaty_stability_shift": f"{random.uniform(-0.18, 0.04):.3f}"
        })
