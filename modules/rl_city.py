...
def render():
    st.markdown("## 🏙️ RL Urban Growth & Multi-Agent Engine")
    ...
    if st.button("🏗️ Execute Urban Growth Simulation Batch", use_container_width=True):
        ...
        # Interactive Plotly Convergence Trajectory
        if rl.city_state_log:
            df_log = pd.DataFrame(rl.city_state_log)
            fig = px.line(df_log, y=["stability", "reward"], title="Urban Growth Convergence Trajectory over Episodes", template="plotly_dark", height=320)
