...
def render():
    ...
    st.markdown("Automated finite element stress analysis, column sizing, and geotechnical foundation design synchronized with the active project.")
    ...
    tributary_area = float(p.get("grid_spacing", 8.0)) ** 2
    ...
    with geo_col1:
        selected_stratum = st.selectbox(
            "Subsurface Geotechnical Stratum",
            [
        ...
        if "380" in selected_stratum:
            bearing_capacity = 380.0
        elif "250" in selected_stratum:
            bearing_capacity = 250.0
        elif "550" in selected_stratum:
            bearing_capacity = 550.0
        else:
            bearing_capacity = 140.0
    ...
    if foundation_mode == "Forced Pad Footings":
        ...
        chosen_foundation = "Isolated Reinforced Concrete Pad Footings" if bearing_capacity >= 250 and floors <= 15 else "Driven Steel H-Pile Caps & Deep Foundation System"
    ...
    f4.metric("Substructure Safety Margin", "1.85", "Concept Check")
    ...
    c1.metric("Column Axial Load (Pᵤ)", f"{column_axial_force:,.1f} kN", f"Area: {tributary_area:.0f} m²")
    ...
    fig.update_layout(
        title=f"Structural Frame Envelope Analysis | {floors} Storeys | {building_height:.1f} m",
