...
def render():
    ...
    if run_sim:
        ...
        carbon_trend = [
            max(250, p.get('carbon_score', 420) * (1 - (i / (sim_generations * 1.5)))) + random.uniform(-10, 10)
            for i in iter_nums
        ]
        cost_trend = [
            p.get('estimated_cost', 49500000) * (1 + (i / (sim_generations * 3.0))) + random.uniform(-100000, 100000)
            for i in iter_nums
        ]
        ...
        fig = px.scatter(
            df_opt,
            x="Generation",
            y="Embodied Carbon (tCO₂e)",
            color="Estimated CAPEX ($)",
            title="Evolutionary Pareto Frontier (Carbon vs Cost Optimization)",
